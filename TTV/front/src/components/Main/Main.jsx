...
function Main() {
  ...
  function handleLike(data) {
    api.likeGuide({
      ...
      .then((res) => setGuides((state) => state.map((guide) => (guide.id === data.id ? res : guide))));
  }
      ...
      <div className="main__guides">
        {
          guides.map((guide, index) => (
            ...
            index === 1 ? <GuideCard data={guide} handleLike={handleLike} key={guide.id} main even /> : <GuideCard data={guide} handleLike={handleLike} key={guide.id} main odd />
          ))
        }
      </div>
    </main>
